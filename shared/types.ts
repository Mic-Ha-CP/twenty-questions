...
export interface Player {
   ...
   * 入房序号,**一房之内唯一且不回收**。
   *
   * ⚠️ 这是本游戏加在登记形状之外的一个字段(PLATFORM 的 generic `Player` 没有它)。
   * 加它的理由:访客名是随机生成的,一屋子「早到的钟表匠」和「早到的收音机」
   * 分不清谁是谁;序号 + 由它派生的固定颜色点,给每个人一个**跨屏一致**的锚。
   * **不回收**是要点 —— 复用号会让「刚才 3 号那句」在同一晚指向两个人。
   * 已记进 NOTES 偏差 log。
   */
  seatNo: number;
}
